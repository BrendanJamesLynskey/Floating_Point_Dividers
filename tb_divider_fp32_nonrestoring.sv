`timescale 1ns / 1ps
/*
    tb_divider_fp32_nonrestoring.sv
    Self-checking testbench for divider_fp32_nonrestoring.sv.
    Identical structure to tb_divider_fp32_restoring.sv — only DUT differs.
    Brendan Lynskey 2025
*/

module tb_divider_fp32_nonrestoring;
...
divider_fp32_nonrestoring u_dut (
...
task automatic run_case(input logic [31:0] a, b);
    ...
    if (timeout >= TIMEOUT_CYCLES) begin
        ...
        if (ref_val == 32'hFFFF_FFFF) begin
            pass_cnt++;  // sentinel: skip exact match
        ...
        end else if (fp32_ulp_dist(tb_q, ref_val) > 1) begin
            $display("FAIL ULP>1: a=%h b=%h got=%h ref=%h ulp=%0d",
                     a, b, tb_q, ref_val, fp32_ulp_dist(tb_q, ref_val)); fail_cnt++;
        end else pass_cnt++;
    end
endtask
